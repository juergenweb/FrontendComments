...
// All functions below are grouped under the FrontendCommentsManager namespace object instead of
// being declared as plain global functions. This avoids polluting the global (window) scope with
// generic names like "docReady" or "checkQueryParam", which could otherwise collide with
// same-named functions from other scripts/plugins loaded on the same admin page (this file and
// assets/frontendcomments.js, for example, both used to declare their own global "docReady" -
// harmless today only because both happened to be identical).
window.FrontendCommentsManager = window.FrontendCommentsManager || {};
...
(function (ns) {
     ...
     * @returns {boolean}
     */
    ns.checkQueryParam = function (field) {
     ...
     * @returns {*|string}
     */
    ns.modifyQueryStringParam = function (uri, key, value) {
     ...
     * @constructor
     */
    ns.removeParameterFromUrl = function (url, parameter) {
        return url
     ...
     * @returns {*}
     */
    ns.getPathFromUrl = function (url) {
        return url.split('?')[0];
    };
     ...
     */
    ns.warnUserUnsavedChanges = function () {
        ...
        const notifyUser = (e) => {
            if (formSubmitting) {
                ...
                if (changes) {
                    ...
                    if (typeof unsavedChangesText !== 'undefined') {
        ...
        let commentForm = document.querySelector('#fcmm-comment-form');
        if (commentForm) {
            commentForm.onsubmit = setFormSubmitting;
        }
    };
     ...
     * @returns {string}
     */
    ns.remove_linebreaks_ss = function (str) {
        let newstr = '';
     ...
     */
    ns.checkFeedbackChange = function () {
        // CKEDITOR is only defined if at least one CKEditor field was rendered on this page
        // (eg. no comments match the current filter -> no feedback field -> CKEDITOR is never loaded)
        ...
        for (let instance in CKEDITOR.instances) {
            ...
            CKEDITOR.instances[instance].on('change', function () {
                ...
                defaultValue = ns.remove_linebreaks_ss(defaultValue);
                currentValue = ns.remove_linebreaks_ss(currentValue);
    ...
    ns.docReady(function () {
        ns.checkStatusChange();
        ns.checkFeedbackChange();
        // check if values have been changed and warn the user
        ns.warnUserUnsavedChanges();

        let currentURL = window.location.href;
        let cleanedUrl = ns.getPathFromUrl(currentURL);
        let urlParts = cleanedUrl.split('/');
        urlParts = urlParts.filter((n) => n);
        let lastItem = urlParts[urlParts.length - 1];
        // Detect the pagination segment (eg. "page2") by its shape instead of counting the total
        // number of URL segments: counting was fragile and gave wrong results depending on the
        // installation's folder depth (eg. ProcessWire installed in a subdirectory instead of
        // directly at the domain root would shift the count and either strip the field-id segment
        // by mistake on page 1, or fail to strip a real pagination segment on page 2+).
        // The pagination segment always ends in a digit but - unlike the preceding, purely
        // numeric field-id segment - is not purely numeric itself, regardless of a customized
        // page-number prefix.
        if (lastItem && /\d$/.test(lastItem) && !/^\d+$/.test(lastItem)) {
                ...
                for (let i = 0; i < formElements.length; i++) {
                    formElements[i].addEventListener('change', function () {
                        ...
                        if (!ns.checkQueryParam(name)) {
                            ...
                            let modifiedCurrentURL = ns.modifyQueryStringParam(currentURL, name, value);
                            //redirect
        ...
        if (tags) {
            ...
            for (let i = 0; i < tagElements.length; i++) {
                tagElements[i].addEventListener('click', function () {
                    ...
                    let url = ns.removeParameterFromUrl(currentURL, filterName);
                    //redirect
...
})(window.FrontendCommentsManager);
